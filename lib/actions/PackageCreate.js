...
usage: serverless package create`,
    ...
    _validateAndPrepare() {
      ...
      if (this.evt.options.names.length) {
        this.evt.options.names.forEach((name) => {
    ...
    _packageFunction(region) {
      ...
      return (func) => {
        ...
        const newEvt = {
          ...
          .then((evt) => {
            if (_.has(S.hooks, 'codeDeployLambdaPre')) {
              return BbPromise.reduce(S.hooks.codeDeployLambdaPre, (acc, hook) => {
                acc.options.pathDist = acc.data.pathDist;
                return hook(acc);
